...
| 12     | `0x1000`  | `00010000 00000000` | `constructor`  | The method is a constructor |
...
The generic type descriptor is used to represent a generic type. It starts with the character 'G', followed by the qualified signature of the owner (class or method), the '#'
character and the name of the generic type. It ends with a ';'.

For example the generic type `T` in the class `shake.lang.List` would be represented as `Gshake/lang/List#T;`.
...
For example the method `shake.lang.String::valueOf(int): String` would be represented as `shake/lang/String:valueOf(I)shake/lang/String`.
...
```
Signature: valueOf(I)shake/lang/String
Qualified Signature: shake/lang/String:valueOf(I)shake/lang/String
```

### Constructor Signature

The constructor signature is similar to the [method signature](#method-signature) but without a return type.

- It's return type is always `N` (for _new_)
- The constructors name is prefixed with a `+` to distinguish it from normal methods.
- There are names constructors. Unnamed constructors are represented as `default`.
...
```
Signature: default([C)N
Qualified Signature: shake/lang/String:+default([C)N
```

For a named constructor:

```shake
package shake.lang;

class String {
    constructor Test(chars: Array<char>) {
        // ...
    }
}
```
...
```
Signature: Test([C)N
Qualified Signature: shake/lang/String:+Test([C)N
```

### Field Signature

The field signature consists of just the name of the field

For example the field `int length` would be represented as `length`.
...
```
Signature: length
Qualified Signature: shake/lang/String:length
```
